...
print("\nÚltimas filas del dataset cargado:")
...
# CORRECCIÓN: Se añade max_iter=5000 para evitar que falle la convergencia
clf_svm = LinearSVC(penalty="l2", dual=False, tol=1e-5, max_iter=5000)
...
print("\nIniciando Grid Search (Por favor, ten paciencia, está entrenando múltiples modelos)...")
penalties = ['l1', 'l2']
tolerances = [1e-3, 1e-4, 1e-5]

param_grid = {'penalty': penalties, 'tol': tolerances}

# CORRECCIÓN: Se agrega max_iter=5000 y loss='squared_hinge' para asegurar compatibilidad y convergencia
grid_search = GridSearchCV(
    LinearSVC(dual=False, loss='squared_hinge', max_iter=5000, random_state=0), 
    param_grid, 
    cv=3
)
...
print(f"\nEntrenando el modelo optimizado definitivo con penalty='{best_penalty}' y tol={best_tol}...")
# CORRECCIÓN: Se añade max_iter=5000 al modelo optimizado final
clf_svm_optimized = LinearSVC(penalty=best_penalty, dual=False, tol=best_tol, loss='squared_hinge', max_iter=5000, random_state=0)
...
print(f'SVM accuracy optimizado final: {acc_svm_opt:.4f}')